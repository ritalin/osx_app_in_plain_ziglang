...
pub const Selector = struct { value: objc.SEL };
...
pub fn MsgSend(comptime T: type) type {
    ...
    return struct {
        ...
        pub fn msgSend(
            ...
            const is_object = Return == objc.id;
            ...
            const msg_send_fn = switch (builtin.target.cpu.arch) {
                ...
                .x86_64 => switch (@typeInfo(RealReturn)) {
                    ...
                    .Int, .Bool, .Pointer, .Void => &c.objc_msgSend,
                    .Optional => |opt| opt: {
            ...
            const Fn = MsgSendFn(RealReturn, @TypeOf(target.value), @TypeOf(sel.value), @TypeOf(args));
            ...
            // var newArgs = .{ target.value, sel.value };
...
fn MsgSendFn(
    ...
    // const Fn = std.builtin.TypeInfo.Fn;
    const FnArg = std.builtin.Type.Fn.Param;

    const params: []FnArg = params: {
        var acc: [argsInfo.fields.len + 2]FnArg = undefined;

        // First argument is always the target and selector.
        acc[0] = .{ .type = Target, .is_generic = false, .is_noalias = false };
        acc[1] = .{ .type = TSelector, .is_generic = false, .is_noalias = false };
...
test {
    ...
    try testing.expectEqual(fn (u8, Selector, u16, u32) callconv(.C) u64, MsgSendFn(u64, u8, @TypeOf(.{
        @as(u16, 0),
        @as(u32, 0),
    })));
}
